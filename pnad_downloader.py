...
BASE_SCHEMA_STRING = 'CREATE TABLE IF NOT EXISTS pnad(id INTEGER PRIMARY KEY AUTOINCREMENT {fields} )'
...
def create_sql_schema(input_dict: dict):
    ...
    with open(f"{BASE_SCHEMA_FOLDER}/schema.sql", "w") as f:
        f.write(BASE_SCHEMA_STRING.replace('{fields}', sql_fields))
...
def read_input(intput_file_path: str) -> dict:
    ...
    with open(intput_file_path) as input_file:
...
def download_url_set(url_set : set) -> set:
    ...
    for url in url_set:
        ...
        if response.status_code != 200:
            print('Link inválido, ignorando arquivo')
            continue
